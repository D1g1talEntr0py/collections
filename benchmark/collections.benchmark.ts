...
import { LinkedMap } from '../dist/linked-map.js';
import { List } from '../src/list.ts';
import { MultiMap } from '../dist/multi-map.js';
import { SetMultiMap } from '../dist/set-multi-map.js';
...
group('LinkedList iteration', () => {
	summary(() => {
		...
		bench('LinkedList singly', () => {
			let total = 0;
			for (const value of singly) { total += value }
			do_not_optimize(total);
		});

		bench('LinkedList doubly', () => {
			let total = 0;
			for (const value of doubly) { total += value }
			do_not_optimize(total);
		});
	});
});
